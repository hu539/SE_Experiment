...
class Evaluate(tk.Tk):
    ...
    def __init__(self, pathone, pathtwo):
        ...
        tk.Tk.__init__(self)
        tk.Tk.wm_title(self, "-- Saturation Enhancement Subjective Assessment --")
        ...
        for idx in self.pages:
            frame = OnePage(idx, pathone, pathtwo, page_size, self.dict, container, self)
...
class OnePage(tk.Frame):
    ...
    def __init__(self, idx, pathone, pathtwo, size, dict_, parent, controller):
...
#            finish.pack(side='bottom', fill='x', padx=ratio*screen_width * 800 / 1728, pady=2)
        ...
        word = "Experiment contact: hu539@purdue.edu\n"
...
#        ratio1 = 0.8
        ...
        self.canvas1 = tk.Canvas(self, bd=0, highlightthickness=2, highlightbackground="black", relief='ridge',xscrollcommand = hbar.set, yscrollcommand=vbar1.set, scrollregion=(-200, -200, new_width2+200, new_height2+200))
        ...
        self.canvas2 = tk.Canvas(self, bd=0, highlightthickness=2, highlightbackground="black", relief='ridge', xscrollcommand = hbar.set, yscrollcommand=vbar2.set, scrollregion=(-200, -200, new_width2+200, new_height2+200))
...
glob_one = os.path.join(PROJECT_ROOT, 'algo1', '*.png')
glob_two = os.path.join(PROJECT_ROOT, 'algo2', '*.png')
...
zip_list = list(zip(images_one, images_two))


unchange_images1 = images_one
unchange_images2 = images_two
exchange_images1, exchange_images2 = [], []

n = len(images_one)

for num in range(math.floor(n/2)):
    temp1, temp2 = random.choice(zip_list)
    ...
    zip_list = list(zip(unchange_images1, unchange_images2))
    
pool_name1 = unchange_images1 + exchange_images1
pool_name2 = unchange_images2 + exchange_images2 
zip_list = list(zip(pool_name1, pool_name2))  
repeat_images1, repeat_images2 = [], []
...
zip_list2 = list(zip(pool_name1_wrepeat, pool_name2_wrepeat))
random.shuffle(zip_list2)
random.shuffle(zip_list2)

final_name1, final_name2 = zip(*zip_list2)


print('\n\t\t GUI window loaded. \n\t\t If the Python window prompt does not appear, please open it manually. \n')
app = Evaluate(final_name1, final_name2)
